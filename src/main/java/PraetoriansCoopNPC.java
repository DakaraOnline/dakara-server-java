...
static final int SONIDO_DRAGON_VIVO = 30;
/* '''ALCOBAS REALES */
...
/* ' */
 ...
 /* ' KINGS */
 TempInt = vb6.val(ES.GetVar(PretorianDat, "KING", "Cantidad"));
 PraetoriansCoopNPC.PretorianAIOffset[ePretorianAI.King] = 1;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "KING", "Alto" + Counter));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "KING", "Bajo" + Counter));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' HEALERS */
 TempInt = vb6.val(ES.GetVar(PretorianDat, "HEALER", "Cantidad"));
 PraetoriansCoopNPC.PretorianAIOffset[ePretorianAI.Healer] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "HEALER", "Alto" + Counter));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "HEALER", "Bajo" + Counter));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' SPELLCASTER */
 TempInt = vb6.val(ES.GetVar(PretorianDat, "SPELLCASTER", "Cantidad"));
 PraetoriansCoopNPC.PretorianAIOffset[ePretorianAI.SpellCaster] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "SPELLCASTER", "Alto" + Counter));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "SPELLCASTER", "Bajo" + Counter));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' SWORDSWINGER */
 TempInt = vb6.val(ES.GetVar(PretorianDat, "SWORDSWINGER", "Cantidad"));
 PraetoriansCoopNPC.PretorianAIOffset[ePretorianAI.SwordMaster] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "SWORDSWINGER", "Alto" + Counter));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "SWORDSWINGER", "Bajo" + Counter));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' LONGRANGE */
 TempInt = vb6.val(ES.GetVar(PretorianDat, "LONGRANGE", "Cantidad"));
 PraetoriansCoopNPC.PretorianAIOffset[ePretorianAI.Shooter] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "LONGRANGE", "Alto" + Counter));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "LONGRANGE", "Bajo" + Counter));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' THIEF */
 TempInt = vb6.val(ES.GetVar(PretorianDat, "THIEF", "Cantidad"));
 PraetoriansCoopNPC.PretorianAIOffset[ePretorianAI.Thief] = PretorianIndex;
  for (Counter = (1); Counter <= (TempInt); Counter++) {
  
  /* ' Alto */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "THIEF", "Alto" + Counter));
  PretorianIndex = PretorianIndex+1;
  /* ' Bajo */
  PraetoriansCoopNPC.PretorianDatNumbers[PretorianIndex] = vb6.val(ES.GetVar(PretorianDat, "THIEF", "Bajo" + Counter));
  PretorianIndex = PretorianIndex+1;
  
 }
 
 /* ' Last */
 PraetoriansCoopNPC.PretorianAIOffset[ePretorianAI.Last] = PretorianIndex;
 
 /* ' Inicializa los clanes pretorianos */
 Declaraciones.ClanPretoriano = new clsClanPretoriano[0];
 Declaraciones.ClanPretoriano = (Declaraciones.ClanPretoriano == null) ? new clsClanPretoriano[1 + 2] : java.util.Arrays.copyOf(Declaraciones.ClanPretoriano, 1 + 2);
 /* ' Clan default */
 Declaraciones.ClanPretoriano[1] = new clsClanPretoriano();
 /* ' Invocable por gms */
 Declaraciones.ClanPretoriano[2] = new clsClanPretoriano();
